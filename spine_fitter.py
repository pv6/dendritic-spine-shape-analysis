...
class SpineGrouping:
    ...
    def save(self, filename: str) -> None:
        with open(filename, "w") as file:
            json.dump({"groups": [list(group) for group in self.groups],
                       "samples": list(self.samples)}, file)

    def load(self, filename: str) -> None:
        with open(filename) as file:
            loaded = json.load(file)
            self.samples = set(loaded["pca_dim"])
...
class SpineFitter(ABC):
    ...
    def show(self, metrics: SpineMetricDataset, groups_to_show: Set[int] = None) -> widgets.Widget:
        out = widgets.Output()
        with out:
    ...
    def _show(self, metrics: SpineMetricDataset, groups_to_show: Set[Any] = None) -> None:
        def show_group(group_label: Any, group: Set[str], color: Tuple[float, float, float, float]) -> None:
        ...
        if groups_to_show is None:
            groups_to_show = set(self.grouping.groups.keys())

        colors = self.grouping.colors
        reduced_data = PCA(2).fit_transform(metrics.as_array())

        for (group_label, group) in self.grouping.groups.items():
            color = colors[group_label] if group_label in groups_to_show else [0.69, 0.69, 0.69, 1]
            show_group(group_label, group, color)
        show_group("Outliers", self.grouping.outlier_group, (0, 0, 0, 1))
                
        plt.title(f"Number of groups: {self.grouping.num_of_groups}")
